...
void emergencyShutdown(ViSession vi, const char* message) {
    ...
    ViStatus ErrorStatus = viPrintf(vi, (ViPRsrc)"OUTP OFF\n");
...
void processVoltageRange(ViSession vi, float limitU_supply_min, float limitU_supply_max, float limitU_supply_step, float limitU_offset, float I_offset) {
    ...
    const int maxOscillations = 5; // Максимальное количество переходов
    
    const float criticalCurrentOffset = 0.05; // Аварийная граница значения тока смещения
    const float criticalCurrentSupply = 0.8; // Аварийная граница значения тока питания
    ...
    for (float current_limitU_supply = limitU_supply_min; current_limitU_supply <= limitU_supply_max; current_limitU_supply += limitU_supply_step) {
        ...
        while (1) {
            ...
            if (measured_current > criticalCurrentSupply) {
            ...
            aboveIOffset = (measured_current > I_offset);
