...
float solveParabolaEquation(float target) {
	...
	if (discriminant < 0) { // Нет реальных решений
		...
		if (x1 > 0) {
			printf("Solutions for the parabola equation = %f\n", x1);
			return x1;
		}
		else if (x2 > 0) {
			printf("Solutions for the parabola equation = %f\n", x2);
